...
#[no_mangle]
pub extern "system" fn Java_io_opendata_log_Log_nativeCreate<'local>(
        ...
        .build()
    {
        Ok(rt) => rt,
        Err(e) => {
            let _ = env.throw_new("io/opendata/log/OpenDataNativeException", e.to_string());
            return 0;
        }
    };
...
#[no_mangle]
pub extern "system" fn Java_io_opendata_log_Log_nativeAppend<'local>(
    mut env: JNIEnv<'local>,
    _class: JClass<'local>,
    handle: jlong,
...
/// JNI function - handle must be a valid pointer returned by nativeCreate.
#[no_mangle]
pub extern "system" fn Java_io_opendata_log_Log_nativeClose<'local>(
    mut env: JNIEnv<'local>,
    _class: JClass<'local>,
    handle: jlong,
) {
...
/// JNI function - must be called from Java with valid JNIEnv.
#[no_mangle]
pub extern "system" fn Java_io_opendata_log_LogReader_nativeCreate<'local>(
    mut env: JNIEnv<'local>,
    _class: JClass<'local>,
    log_handle_ptr: jlong,
) -> jlong {
...
/// JNI function - handle must be a valid pointer returned by nativeCreate.
#[no_mangle]
pub extern "system" fn Java_io_opendata_log_LogReader_nativeRead<'local>(
    mut env: JNIEnv<'local>,
    _class: JClass<'local>,
    handle: jlong,
    key: JByteArray<'local>,
...
/// JNI function - handle must be a valid pointer returned by nativeCreate.
#[no_mangle]
pub extern "system" fn Java_io_opendata_log_LogReader_nativeClose<'local>(
    _env: JNIEnv<'local>,
    _class: JClass<'local>,
    handle: jlong,
) {
...
fn create_append_result<'local>(
    ...
    let class = env.find_class("io/opendata/log/AppendResult")?;
...
fn create_log_entry_array<'local>(
    ...
    let class = env.find_class("io/opendata/log/LogEntry")?;
